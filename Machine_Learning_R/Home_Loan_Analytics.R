...
#ggplot(data=Home_Loan_Raw2,aes(TotalIncome,fill=Married))+geom_histogram(binwidth = 5000,position="dodge")+facet_wrap(Gender~.)
...
nums <- sapply(Home_Loan_Raw2,class)=="numeric"
numvars <- Home_Loan_Raw2[,nums]
m<-cor(numvars)
#v<-as.vector(m) 
...
HL_train <- Home_Loan_Raw2[Home_Loan_Train, ]
HL_test <- Home_Loan_Raw2[-Home_Loan_Train, ]

feature.names=names(HL_train)

for (f in feature.names) {
  if (class(HL_train[[f]])=="factor") {
    levels <- unique(c(HL_train[[f]]))
    HL_train[[f]] <- factor(HL_train[[f]],
...
newtrain <- cbind(Loan_Status=HL_train$Loan_Status,Home_Loan_Raw2[1:429,])

#bogus Loan status for test set
Loan_Status <- as.factor(sample(c("N","Y"),replace=TRUE,size=dim(HL_test)[1]))
newtest <- cbind(Loan_Status,Home_Loan_Raw2[430:614,])
...
submit1 <- data.frame(Loan_ID = HL_test$Loan_ID, Loan_Status = treepred$data$response)
